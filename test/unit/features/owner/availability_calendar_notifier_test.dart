...
void main() {
  ...
  group('blockDates', () {
    test('calls repository with date list and sets success message', () async {
      when(() => mockRepository.blockSlots('hall-1', any()))
      ...
      final captured =
          verify(() => mockRepository.blockSlots('hall-1', captureAny()))
              .captured
              .first as List<DateTime>;
      expect(captured.length, 3);
      expect(captured[0].day, 15);
      expect(captured[1].day, 16);
      expect(captured[2].day, 17);
    ...
    test('sets error when blocking fails due to booked slots', () async {
      when(() => mockRepository.blockSlots('hall-1', any())).thenAnswer(
        (_) async => const Left(
  ...
  group('unblockDates', () {
    test('calls repository and sets success message', () async {
      when(() => mockRepository.unblockSlots('hall-1', any()))
          .thenAnswer((_) async => const Right(null));
      when(() => mockRepository.getSlotSummary('hall-1', any(), any()))
          .thenAnswer((_) async => const Right({}));
      ...
      final captured =
          verify(() => mockRepository.unblockSlots('hall-1', captureAny()))
              .captured
              .first as List<DateTime>;
